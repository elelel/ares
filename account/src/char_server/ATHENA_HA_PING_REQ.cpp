...
void ares::account::char_server::packet_handler<ares::packet<ares::packets::ATHENA_HA_PING_REQ>>::operator()() {
  ...
  if (server_state_.server.char_servers().find(session_.shared_from_this()) !=
    ...
    session_.emplace_and_send<packet<packets::ATHENA_AH_PING_ACK>>();
  } else {
