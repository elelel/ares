...
void ares::account::mono::packet_handler<ares::packet_set, ares::packet::CA_SSO_LOGIN_REQ::login_password>::accept(const std::string& login) {
  ...
  if (user_data) {
    ...
    SPDLOG_TRACE(log(), "CA_SSO_LOGIN_REQ::login_password accept: acquiring server lock");
    std::lock_guard lock(server_state_.server.mutex());
    SPDLOG_TRACE(log(), "CA_SSO_LOGIN_REQ::login_password accept: server lock acquired");
