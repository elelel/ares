...
auto ares::account::session::packet_sizes(uint16_t& packet_id) -> std::tuple<size_t, size_t, size_t> {
  struct visitor {
...
void ares::account::session::dispatch_packet(std::shared_ptr<std::byte[]> buf) {
  struct visitor {
