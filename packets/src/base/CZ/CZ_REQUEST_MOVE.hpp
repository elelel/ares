struct CZ_REQUEST_MOVE {
  inline void emplace(const uint16_t to_x, const uint16_t to_y) {
    PacketType = packet_id::CZ_REQUEST_MOVE;
    dest_[0] = to_x >> 2;
  ...
  explicit CZ_REQUEST_MOVE(const uint16_t to_x, const uint16_t to_y) {
    emplace(to_x, to_y);
  }

  explicit CZ_REQUEST_MOVE(const uint16_t to_x, const uint16_t to_y, uint8_t dir) {
