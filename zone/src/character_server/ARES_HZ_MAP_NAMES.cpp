...
void ares::zone::character_server::packet_handler<ares::packet::ARES_HZ_MAP_NAMES>::operator()() {
  SPDLOG_TRACE(log(), "handle_packet ARES_HZ_MAP_NAMES: begin");
  if (p_->PacketLength != sizeof(ares::packet::ARES_HZ_MAP_NAMES)) {
    const size_t max_i = p_->PacketLength - sizeof(ares::packet::ARES_HZ_MAP_NAMES);
    ...
    session_.emplace_and_send<packet::ARES_ZH_MAP_NAMES_REQ>(session_.recv_unfragmented_free_size() / 2);
  } else {
