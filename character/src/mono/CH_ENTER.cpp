...
void ares::character::mono::packet_handler<ares::packet_set, ares::packet::CH_ENTER>::operator()() {
...
void ares::character::mono::packet_handler<ares::packet_set, ares::packet::CH_ENTER>::from_zone_server(session_ptr) {
  //  SPDLOG_TRACE(server_state_.log(), "CH_ENTER - transforming from existing session {:#x}", (uintptr_t)existing_session.get());
  // TODO:
}

void ares::character::mono::packet_handler<ares::packet_set, ares::packet::CH_ENTER>::from_account_server() {
  ...
  server.account_server()->emplace_and_send<packet_type<packet::ATHENA_HA_AID_AUTH_REQ>>(p_->AID(),
                                                                             p_->AuthCode(),
...
void ares::character::mono::packet_handler<ares::packet_set, ares::packet::CH_ENTER>::refuse(const uint8_t error_code) {
  session_.emplace_and_send<packet_type<packet::HC_REFUSE_ENTER>>(error_code);
  throw ares::network::terminate_session();
}

void ares::character::mono::packet_handler<ares::packet_set, ares::packet::CH_ENTER>::notify_ban(const uint8_t error_code) {
