    ...
    if (ec.value() == 0) {
      ...
      session_->socket()->async_connect(ep, [session_copy] (const std::error_code ec) {
          ...
          if (ec.value() == 0) {
            ...
            session_copy->emplace_and_send<packet_type<packet::ATHENA_HA_LOGIN_REQ>>(conf.account_server->login,
...
void ares::character::account_server::timer::ping_request::on_timer() {
  ...
  if (server.account_server() &&
    ...
    session_->emplace_and_send<packet_type<packet::ATHENA_HA_PING_REQ>>();
