    ...
    if (ec.value() == 0) {
      ...
      session_->socket()->async_connect(ep, [session_copy] (const std::error_code ec) {
          ...
          if (ec.value() == 0) {
            ...
            session_copy->emplace_and_send<packet<packets::ATHENA_HA_LOGIN_REQ>>(conf.account_server->login,
...
void ares::character::account_server::timer::send_aids::on_timer() {
  ...
  session_->emplace_and_send<packet<packets::ATHENA_HA_ONLINE_AIDS>>(session_->server_state_.server.clients().size());
...
void ares::character::account_server::timer::send_user_count::on_timer() {
  std::lock_guard<std::mutex> lock(session_->server_state_.server.mutex());
  session_->emplace_and_send<packet<packets::ATHENA_HA_USER_COUNT>>(session_->server_state_.server.clients().size());
...
void ares::character::account_server::timer::ping_request::on_timer() {
  ...
  if (server.account_server() &&
    ...
    session_->emplace_and_send<packet<packets::ATHENA_HA_PING_REQ>>();
