...
void ares::character::account_server::packet_handler<ares::packet_set, ares::packet::ATHENA_AH_AID_AUTH_RESULT>::operator()() {
  ...
  if (s != nullptr) {
    if (p_->fail() == 0) {
      if (std::holds_alternative<mono::state>(s->session_state_)) {
        ...
        session_.emplace_and_send<packet_type<packet::ATHENA_HA_ACCOUNT_DATA_REQ>>(p_->aid());
      } else {
      ...
      s->emplace_and_send<packet_type<packet::AC_REFUSE_LOGIN>>(8); // Rejected by server
      std::lock_guard<std::mutex> lock(server.mutex());
      s->remove_from_server();
    }
  } else {
