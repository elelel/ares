...
void ares::character::zone_server::packet_handler<ares::packet::ARES_ZH_MAP_NAMES_REQ>::operator()() {
  ...
  if (maps_to_send.size() > 0) {
    ...
    if (max_batch_sz > 20) {
      ...
      session_.emplace_and_send<packet::ARES_HZ_MAP_NAMES>(batch_sz);
      size_t sz{0};
    ...
    session_.emplace_and_send<packet::ARES_HZ_MAP_NAMES>(0);
  }
