    ...
    if ((data.auth_code1 == p_->auth_code1()) && (data.aid == p_->aid())) {
      if (data.char_info) {
        if (data.char_info->cid == p_->cid()) {
          ...
          session_.emplace_and_send<packet::current<packet::ARES_HZ_CID_AUTH_RESULT>>(p_->request_id(), p_->cid(), data.auth_code2, 0);
        } else {
          log()->warn("Failed authentication attempt from zone active aid {} with cid {}, cid mismatch", p_->aid(), p_->cid());
          session_.emplace_and_send<packet::current<packet::ARES_HZ_CID_AUTH_RESULT>>(p_->request_id(), p_->cid(), 0, 1);
        }
      } else {
